...
execute as @a[tag=!ar_temp_immu,sort=random,limit=3] run tag @s add ar_temp_victim
execute as @a[tag=ar_temp_victim] run function acerace4:modifiers/wildcard/effect/sendback1

# Summary
tellraw @s ["§6[§r彀§6] 道具结算：",{"selector":"@a[tag=ar_temp_victim]"},"§c被往回传送了一个存档点。"]
