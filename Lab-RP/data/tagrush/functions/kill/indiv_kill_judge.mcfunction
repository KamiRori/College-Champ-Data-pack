...
execute as @s run function tagrush:kill/friendlykill

# Decides if there is a valid killer
execute as @s run function tagrush:kill/findkiller

# Player Kill
execute as @s[tag=!tagrush_nokiller] run function tagrush:kill/killmsg

# Nature Damage
execute as @s[tag=tagrush_nokiller] run function tagrush:kill/nature

tag @s remove queue_kill_judge
tag @s remove tagrush_alive
tag @s add tagrush_round_eliminated
