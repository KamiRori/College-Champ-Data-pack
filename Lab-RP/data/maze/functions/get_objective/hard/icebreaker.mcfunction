...
execute as @e[type=marker,tag=maze_objective,tag=difficulty_hard,tag=type_icebreaker,tag=!state_inuse,limit=1,sort=random] run tag @s add selected_challenge
...
execute as @e[type=marker,tag=maze_objective,tag=selected_challenge] at @s run function maze:objective/icebreaker/setup_hard

# FX
title @s times 5 30 5
title @s title "§b§l破冰挑战§4§l +"
title @s subtitle "将§b左边§f的冰雕复制到§6右边"

tellraw @s ["§c[§r鼱§c] ","§c房间难度已被强化！"]
# Time Limit
scoreboard players operation @s maze_objective_time_limit = $icebreaker_hard maze_objective_time_limit
