...
execute as @e[type=marker,tag=maze_objective,tag=difficulty_normal,tag=type_search,tag=!state_inuse,limit=1,sort=random] run tag @s add selected_challenge
...
execute as @e[type=marker,tag=maze_objective,tag=selected_challenge] at @s run function maze:objective/search/setup_normal

# FX
title @s times 5 30 5
title @s title "§6§l搜索挑战"
...
# Time Limit
scoreboard players operation @s maze_objective_time_limit = $search_normal maze_objective_time_limit
